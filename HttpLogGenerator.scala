...
class HttpLogGenerator(val ipGenObj: IPGenerator, var messagesCount: Int = 1) {
  val EXTENSIONS = 	mutable.Map(
    "html" -> 40,
    "php" -> 30,
    "png" -> 15,
    "gif" -> 10,
  ...
  val RESPONSE_CODES = mutable.Map(
    "200" -> 92,
    "404" -> 5,
    "503" -> 3
  )

  val USER_AGENTS = mutable.Map(
...
object RandomHttpLogGen extends App {
  ...
  try {
    ...
    new HttpLogGenerator(new IPGenerator(100, 10)).writeMps(outputFileWriter, messagesPerSec)
  } catch {
    case e: Exception => e.printStackTrace()
  } finally {
    println("closing file")
